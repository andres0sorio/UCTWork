...
void makePlots( const char * varname, const char * varSel, const char * binning, float minPU, float maxPU, float calibFactor ) {
  ...
  if ( 1 ) { 
    ...
    input_data = TString("../uct_eff_trees_jetskim.root"); //jet_skim is a skim produced by the WI group (is a MuTau skim + some additional selection)
    path << "./uct15-plots/efficiency/Jets/noPUC/isoStudies";
  ...
  L1UCTEfficiency * eg1   = new L1UCTEfficiency(input_data.Data(),"jetEfficiency/Ntuple");
    
  // Jets - ok
  dSel << "(" << varSel << ") ";
  l1Sel << "(" << varSel << ") && l1Match  && l1Pt >= " << l1PtCut;
  l1gSel << "(" << varSel << ") && l1gMatch && l1gPt >= " << l1PtCut*calibFactor;
  ...
  for( int k = 0; k < max; ++k ) {
    ...
    eg1->Loop(varname, binning, "Jets Efficiency","eg1");
    
    effgraph = new EffGraph(eg1->GetEff("l1Eff"));
    effgraph->SetText("Trigger Efficiency: Jet20", XaxisLabel.Data(),"Current");
    v_efficiencies->Add( effgraph );
    
    effgraph = new EffGraph(eg1->GetEff("l1gEff"));
    effgraph->SetText("Trigger Efficiency: Jet20",XaxisLabel.Data(),((TObjString *)v_SelectionNames->At(k))->GetString().Data() );
    v_efficiencies->Add( effgraph );
    
    //Compare current vs Selection
    saveAs << path.str() << "/png/rlx_jets_eff_trg20_" << varname  << "_" << minPU << "_" << maxPU << "_" 
	   << k << "_" << PUcut << "_" << calibFactor << ".png";

    eg1->ComparePlots(v_efficiencies,"EG Efficiency", saveAs.str().c_str() );
    
    saveAs.str("");
    saveAs << path.str() << "/eps/rlx_jets_eff_trg20_" << varname  << "_" << minPU << "_" << maxPU << "_" 
	   << k << "_" << PUcut << "_" << calibFactor << ".eps";

    eg1->ComparePlots(v_efficiencies,"Jet Efficiency", saveAs.str().c_str() );
