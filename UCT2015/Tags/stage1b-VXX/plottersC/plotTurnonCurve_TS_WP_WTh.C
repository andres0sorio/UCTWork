...
void plotTurnonCurve(char* var, double min, double max, int NPVmin=0, int NPVmax=50) {
  ...
  for( int k = min_sel; k < max_sel; ++k ) {
    ...
    TLegend* legend = new TLegend(0.5,0.22,0.9,0.45,"95% offline thresholds:" );
    legend->SetFillColor(0);
    legend->SetBorderSize(0);
    legend->SetTextFont(62);
    legend->SetTextSize(0.034);
    legend->AddEntry(l1Pt20, "29 GeV","PLE"); 
    legend->AddEntry(l1Pt25, "35 GeV", "PLE"); 
    legend->AddEntry(l1Pt30, "41 GeV", "PLE"); 
    legend->AddEntry(l1Pt40, "53 GeV", "PLE"); 
    legend->AddEntry(l1Pt60, "77 GeV", "PLE"); 
    legend->Draw();
    cmsPrelim();
    canEffIso->SetGridx();
    canEffIso->SetGridy();
    ...
    saveAs.str("");
    saveAs << path << "/eps/rlx_tau_eff_trg20_WP_Wth_" << varName << "_" << NPVmin << "_" << NPVmax << "_" << k << "_" << PUcut 
	   << "_" << lineWidth << ".eps";
    ...
    saveAs.str("");
    saveAs << path << "/png/rlx_tau_eff_trg20_WP_Wth_" << varName << "_" << NPVmin << "_" << NPVmax << "_" << k << "_" << PUcut 
	   << "_" << lineWidth << ".png";
    canEffIso->SaveAs( saveAs.str().c_str() );
