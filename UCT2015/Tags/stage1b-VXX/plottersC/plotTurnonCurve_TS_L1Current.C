...
void plotTurnonCurve() 
{
  ...
  plotTurnonCurve("Pt",0.0,120.0, plot_all);

  /// plotTurnonCurve("Eta",0.0,120.0, plot_all);
  
}

void plotTurnonCurve(char* var, double min, double max, bool plotAll) {
  ...
  if( 1 ) {
    ...
    f1 =  TFile::Open("../uct_efficiency_tree_mutauWI_Skim_1.23.root");
  ...
  std::ifstream *m_in = new std::ifstream("config/efficiency_TS_WP_WI_Selection.dat"); //AO:
  ...
  TObjString *label;
  label = new TObjString( "L1 Upgrade Tau (solid)" );
  v_Labels->Add( label ); 
  label = new TObjString( "L1 Upgrade Iso Tau (solid)" );
  v_Labels->Add( label ); 
  
  TObjString *label_current;
  label_current = new TObjString( "L1 Current Tau (dashed)" );
  v_Labels_Current->Add( label_current ); 
  label_current = new TObjString( "L1 Current Iso Tau (dashed)" );
  v_Labels_Current->Add( label_current ); 

  ////////////////////////////////////////////////////////////////////////////////
  
  /// L1 Upgrade base selection
  char cuts[100];
  sprintf(cuts, " l1gMatch && recoPt > %d ", RecoPtmin);
  
  /// L1 Current base selection
  char l1cuts[100];
  sprintf(l1cuts, " l1Match && recoPt > %d ", RecoPtmin);
  ...
  for( int k = min_sel; k < max_sel; ++k ) {
    ...
    // any further selection is read from file - for instance in case of isolation(s)
    // now for Taus it reads a "1" ie nothing is introduced here.
    ...
    TH1D* l1PtCt20 = allReco->Clone("l1PtCt20");
    l1PtCt20->SetLineStyle(2);
    l1PtCt20->SetLineColor(1);
    l1PtCt20->SetMarkerColor(1);
    
    TH1D* l1PtCt25 = allReco->Clone("l1PtCt25");
    l1PtCt25->SetLineStyle(2);
    l1PtCt25->SetLineColor(6);
    l1PtCt25->SetMarkerColor(6);
    
    TH1D* l1PtCt30 = allReco->Clone("l1PtCt30");
    l1PtCt30->SetLineStyle(2);
    l1PtCt30->SetLineColor(7);
    l1PtCt30->SetMarkerColor(7);
    
    TH1D* l1PtCt40 = allReco->Clone("l1PtCt40");
    l1PtCt40->SetLineStyle(2);
    l1PtCt40->SetLineColor(8);
    l1PtCt40->SetMarkerColor(8);
    
    TH1D* l1PtCt60 = allReco->Clone("l1PtCt60");
    l1PtCt60->SetLineStyle(2);
    l1PtCt60->SetLineColor(2);
    l1PtCt60->SetMarkerColor(2);
    ...
    tree1->Draw(varName+TString(">>allReco"),"recoPt > 0.0","goff"); //Denominator

    // Numerators - L1 Upgrade
    ...
    tree1->Draw(varName+TString(">>l1PtCt20"),TString(l1cuts)+TString(" && l1Pt>20. && ")+egsel,"goff");
    tree1->Draw(varName+TString(">>l1PtCt25"),TString(l1cuts)+TString(" && l1Pt>25. && ")+egsel,"goff");
    tree1->Draw(varName+TString(">>l1PtCt30"),TString(l1cuts)+TString(" && l1Pt>30. && ")+egsel,"goff");
    tree1->Draw(varName+TString(">>l1PtCt40"),TString(l1cuts)+TString(" && l1Pt>40. && ")+egsel,"goff");
    tree1->Draw(varName+TString(">>l1PtCt60"),TString(l1cuts)+TString(" && l1Pt>60. && ")+egsel,"goff");
    ...
    l1PtCt60->Divide(allReco);
    
    //------------------------------------------------------------------------

    TCanvas* canEffIso = new TCanvas("canEffIso","",575,57,718,680);
    ...
    TString legend_Label_Current = ((TObjString*)v_Labels_Current->At(k))->GetString();
    ...
    if (plotAll ) {
      tex = new TLatex(14.25,1.10, legend_Label.Data() );
      tex->SetTextFont(42);
      ...
      tex = new TLatex(4.49,1.29, legend_Label_Current.Data() );
      tex->SetTextFont(42);
      tex->SetTextSize(0.04);
      tex->SetLineWidth(2);
      tex->Draw();
    ...
    saveAs.str("");
    saveAs << path << "/eps/rlx_tau_eff_trg20_All_" << varName << "_" << k << "_" << lineWidth << ".eps";
    canEffIso->SaveAs( saveAs.str().c_str() );
    
    saveAs.str("");
    saveAs << path << "/eps/rlx_tau_eff_trg20_All_" << varName << "_" << k << "_" << lineWidth << ".pdf";
    canEffIso->SaveAs( saveAs.str().c_str() );
    
    saveAs.str("");
    saveAs << path << "/png/rlx_tau_eff_trg20_All_" << varName << "_" << k << "_" << lineWidth << ".png";
    canEffIso->SaveAs( saveAs.str().c_str() );
    
    break; // we are testing -> remove when there is no need
