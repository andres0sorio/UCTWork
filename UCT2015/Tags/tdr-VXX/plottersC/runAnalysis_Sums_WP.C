...
void makePlots(const char * varname, const char * binning, float minPU, float maxPU, float PUCut ) {
  ...
  if( debug ) {
    ...
    sprintf(filenamePNG, "./uct15-plots/rate/Sums/%s/isoStudies/png/rlx_sums_%s_%0.2f.png" , PUopt.Data(), varname, PUCut);
    sprintf(filenameEPS, "./uct15-plots/rate/Sums/%s/isoStudies/eps/rlx_sums_%s_%0.2f.eps" , PUopt.Data(), varname, PUCut);
