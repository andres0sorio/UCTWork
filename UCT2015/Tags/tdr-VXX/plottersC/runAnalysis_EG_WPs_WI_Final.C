...
void makePlots( const char* binning, float PUCut ) {
  ...
  float cuts[3] = { 0.20 }; //  0.20 is the TDR Working Point
  ...
  for( int k=0; k<1; ++k) {
    ...
    sprintf(filenameCpp, "./uct15-plots/rate/EG/%s/isoStudies/png/rlx_eg_Iso_CMP_%0.2f_%0.2f.C"   , PUopt.Data(), isoCut1, PUCut );
    ...
    JetIsolation.str("");
    JetIsolation << BaseSelection.str() << " && 1 ";
    ...
    if( debug ) { 
      ...
      TString plot_name("uct_regId_noniso");
      ...
      ratehisto = new RateHisto( uct->Get( plot_name.Data() ) );
      ratehisto->SetText("Trigger Rate: EG20","L1 PT (GeV)","2x1 E+H Cluster & regionalId ");
      //v_singleCMP->Add( ratehisto );
