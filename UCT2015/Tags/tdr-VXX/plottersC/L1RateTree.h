...
class L1RateTree : public Histograms {
 public :
  ...
  virtual void EvalNormalization(UInt_t minLS, UInt_t maxLS );

  void SetNormalizationConstants(float desiredLumi, float preScale, float deltaLS ) {

    DESIRED_LUMI = desiredLumi; // 200.0; // in units of 10^32 cm-2 s-1
    PSF = preScale; // 2.0 * 23.0 * 4.0; // Prescale factor = 92 ok x 2 L1  prescale
    LS_DELTA_T = deltaLS; // 23.3570304; // Default value
