...
void plotResolution( ) {
  ...
  std::ofstream * logfileSt1 = new std::ofstream( (log_subdir + TString("reso_vs_pvs_Stg1.csv")).Data() );
  std::ofstream * logfileSt2 = new std::ofstream( (log_subdir + TString("reso_vs_pvs_Stg2.csv")).Data() );
  ...
  if (region == 1 ) {
    ...
    plotResolution("Pt", 0, maxPv, "( abs(recoEta) > 1.6 && abs(recoEta) < 2.4 )" , "(50,-2.0 , 2.0)", "MuTau_MTcut_EC", region, 
