...
void plotResolution( ) {
  ...
  std::ofstream * logfileSt1 = new std::ofstream( (log_subdir + TString("reso_vs_pt_Stg1.csv")).Data() );
  std::ofstream * logfileSt2 = new std::ofstream( (log_subdir + TString("reso_vs_pt_Stg2.csv")).Data() );
  ...
  if (region == 1 ) {
    ...
    while ( ptTwo < maxPt ) {
      ...
      plotResolution("Pt", ptOne, ptTwo, "( abs(recoEta) < 1.4 )", "(50,-2.0 , 2.0)", "MuTau_MTcut_EC", region, logfileSt1, logfileSt2 );
      
      std::cout << " xmin " << ptOne << " xmax " << ptTwo << std::endl;
    ...
    plotResolution("Pt", 0, maxPt, "( abs(recoEta) > 1.6 && abs(recoEta) < 2.4 )" , "(50,-2.0 , 2.0)", "MuTau_MTcut_EC", region, 
...
void plotResolution( char* var, float xmin, float xmax, const char * region, const char *xbinning, const char * skim, int regionCode, 
  ...
  TString log_subdir("./logs/");
  TString resolution_log = TString("resolution_Ptbins_") + TString(var) + TString("_") + TString(skim) + TString(".log");
  ...
  for( int k = 0 ; k < 1; ++k ) {
    ...
    sprintf(cuts, " ( l1Pt >= 25.0 && l1gMatch >= 1.0 )&& %s && recoPt > %d && recoPt < %d && decayMode >= %d", region, xMin, xMax, decayMode[k]);
    ...
    h1->Draw("HIST");
    fitFun = (TF1*)h1->GetListOfFunctions()->FindObject("g1");
    fitFun->Draw("same");
    canvas->Update();
    ...
    ps1->SetName("statsh1");
    ps1->SetTextColor(2);
    ps1->SetY1NDC( ps1->GetY1NDC() - 0.10 ); 
    ps1->SetY2NDC( ps1->GetY2NDC() - 0.10 );
    ps1->SetFillColor(0);
    ps1->SetFillStyle(1001);
