...
void makePlots() {
  ...
  //Pt
  if( 1 ) {
    makePlots("recoPt", "recoPt > 0.0", "40,0,100", 0.0, 100);
  }
...
void makePlots( const char * varname, const char * varSel, const char * binning, float minPU, float maxPU ) {
  ...
  if ( 1 ) { 
    ...
    input_data = TString("../uct_eff_trees_mutau_BC.root");
    path << "./slhc-plots/efficiency/Taus/noPUC/isoStudies";
  ...
  std::ifstream *m_in = new std::ifstream("config/efficiency_TS_WP_Selection.dat");
