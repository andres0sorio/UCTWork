...
void makePlots() {
  ...
  TGraphErrors * graphT3 = readTypeThree("logs/reso_vs_pvs_Stg1.csv");
  allgraphs->Add( graphT3 ); //Append to your List this Graph
  ndataset++;
  
  graphT3 = readTypeThree("logs/reso_vs_pvs_Stg2.csv");
  allgraphs->Add( graphT3 ); //Append to your List this Graph
  ndataset++;
  
  graphT3 = readTypeThree("logs/reso_vs_pvs_Stg1MC.csv");
  allgraphs->Add( graphT3 ); //Append to your List this Graph
  ndataset++;
  
  graphT3 = readTypeThree("logs/reso_vs_pvs_Stg2MC.csv");
  allgraphs->Add( graphT3 ); //Append to your List this Graph
  ndataset++;
  ...
  for(int k=0; k < ndataset; ++k) {
    ...
    if( k == 0 ) {
      ...
      ((TGraph*)allgraphs->At(k))->GetXaxis()->SetTitle("n PVs");
  ...
  saveAs.str("");
  saveAs << path << "/eps/rlx_resolution_PVbins" << ".eps";
  canvas->SaveAs( saveAs.str().c_str() );
  
  saveAs.str("");
  saveAs << path << "/pdf/rlx_resolution_PVbins" << ".pdf";
  canvas->SaveAs( saveAs.str().c_str() );

  saveAs.str("");
  saveAs << path << "/png/rlx_resolution_PVbins" << ".png";
  canvas->SaveAs( saveAs.str().c_str() );
